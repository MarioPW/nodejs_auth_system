...
### **Startup Process**
When the application starts, it executes the following operations in order:

1. **🌐 Environment Variables Loading**
  - Loads all configuration from `.env` file using `dotenv`.
  - Variables must be properly configured before starting the server.

2. **🗄️ Database Connection**
  - Tests connection to the database using credentials from `DB_HOST`, `DB_PORT`, `DB_USER`, `DB_PASSWORD`, and `DB_NAME`.
  - If the connection fails, the server will not start.

3. **📂 Database Synchronization**
  - Executes `sequelize.sync({ force: false })` to create tables if they don't exist:
    - **Users**: Stores user information (email, password, OAuth IDs, roles, etc.).
    - **RolesLookup**: Stores available system roles.
  - `force: false` ensures existing tables are **not dropped** (safe for production). Only creates tables that don't exist, preserving existing data.

4. **🔑 Roles Initialization**
  - Inserts roles defined in `APP_ROLES` into the `RolesLookup` table.
  - Only adds missing roles (avoids duplicates).
  - Roles must match those used across your entire system.

5. **📧 SMTP Configuration Test**
  - Validates email configuration (`SMTP_EMAIL`, `SMTP_EMAIL_PASSWORD`, `SMTP_HOST`).
  - Tests connection to the email server for password reset functionality.
  - **Non-blocking**: Server starts even if SMTP fails.

6. **🚀 Server Startup**
  - Starts the HTTP server on the port specified in `PORT` (default: `3000`).
...
#### **Must Be Identical Across Your System**
| Variable       | Requirement                          | Why                                                                 |
|----------------|--------------------------------------|---------------------------------------------------------------------|
| `🔒 JWT_SECRET`   | Must be the same across **all APIs** | Ensures tokens generated by the Auth API can be validated by others. |
| `🗄️ DB_NAME`      | Should be the same if using shared database | Multiple APIs connecting to the same database must use the same name. |
| `🔑 APP_ROLES`    | Must include all roles used system-wide | Ensures role validation works consistently across all services.      |

---

### **📊 Database Behavior**
- **`force: false`**: Tables are created only if they don't exist (**safe mode**).
- If you need to recreate tables (**⚠️ data loss**), change to `force: true`.
- Two main tables created automatically:
  - **Users**: Authentication and user management.
  - **RolesLookup**: System roles catalog.

---

### **❗ Error Handling**
- **❌ Database connection fails** → Server stops (**critical error**).
- **⚠️ SMTP validation fails** → Server continues (**warning only**).
- **🚨 Missing environment variables** → May cause runtime errors.

---

### 🔄 Integration with Other Systems
If integrating this Auth System with other APIs (e.g., Medical Appointments API):
- Share the same `🔒 JWT_SECRET` for token validation.
- Use the same database (`🗄️ DB_NAME`, `DB_HOST`, etc.) if using **Option 1 architecture**.
- Coordinate `🔑 APP_ROLES` to match roles used in both systems.
- Import authentication middleware in other APIs to validate tokens.
...
✅ All required roles found in database correctly (3 roles)
✅ SMTP ready
