    ...
    private async sendTestEmail(transporter: nodemailer.Transporter, config: SMTPConfig): Promise<boolean> {
        try {
            const result = await transporter.sendMail({
                ...
                html: `
                <h3 style="font-family: Arial, sans-serif; color: #333;">SMTP Test - ${config.name}</h3>
                <table border="1" cellpadding="5" cellspacing="0" style="border-collapse: collapse; width: 50%; text-align: left; font-family: Arial, sans-serif; color: #333; background-color: #f9f9f9; border: 1px solid #ddd;">
